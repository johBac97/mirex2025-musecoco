...
Compare to enc_remigen_utils, this version
- Support tempo
- Support newer version of miditoolkit

- [2024.2.7] miditoolkit.containers -> miditoolkit
'''
import miditoolkit
...
def convert_pos_info_to_token_lists(
    ...
    for pos in range(max_pos):
        ...
        # When entering a new bar, add a bar line token, 'b-1'
        if cur_bar != now_bar:
            if cur_bar is not None:
                encoding.append((const.BAR_ABBR, 1))  # bar
            cur_bar = now_bar

            # The first token of each bar is time signature.
            if not ignore_ts:
                encoding.append((const.TS_ABBR, cur_ts))  # ts

            # Second token is tempo
            if not ignore_tempo:
    ...
    # Append a bar line token in the end
    encoding.append((const.BAR_ABBR, 1))  # bar
...
def do_remove_empty_bars(encoding, ignore_ts=False):
    '''
    Longshen: this function is untested. Might be buggy.
    '''
...
def generate_midi_obj_from_remigen_token_list(
    ...
    for item in token_list:  # Todo: 对错误的结果顺序做处理
        ...
        if item_type == const.BAR_ABBR:
            ...
            if last_item_type != const.PITCH_ABBR:
                print('Warning: duration token without pitch token')
            # assert last_item_type == const.PITCH_ABBR, 'last item should be {} but is {}'.format(const.PITCH_ABBR, last_item_type)
            ...
            if ignore_velocity:
                print('There are velocity token but setting ignor_velocity=True')
                # raise ValueError(
    ...
    # If there is no time signature change, add the default one
