    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # print(outputs.logits.size(1))
            # print(probs.size())
            # print(-((-next_token_scores[0]).sort()[0])[0: 2])
            # print(-((-probs[0]).sort()[0])[0: 2])
            # print(((-probs[0]).sort()[1])[0: 2])

            # pp = torch.multinomial(-((-probs[0]).sort()[0])[0: 2], num_samples=1)
            # if pp.cpu().item() == 0:
            #     next_tokens = ((-probs[0]).sort()[1])[0: 2][0]
            # elif pp.cpu().item() == 1:
            #     next_tokens = ((-probs[0]).sort()[1])[0: 2][1]

            next_tokens = torch.multinomial(probs, num_samples=1).squeeze(1)

            # print(next_tokens, probs[0, next_tokens.cpu().item()])

            # cou = 0
            # for _ in range(100):
            #     # print(probs.size())
            #     next_tokens_ = torch.multinomial(probs[0], num_samples=1)  # .squeeze(1)
            #     if next_tokens_.cpu().item() == ((-probs[0]).sort()[1])[0]:
            #         cou +=1
            #     if _ == 4:
            #         print(cou / 5)
            # print(cou / 100)
            #     # print(next_tokens_, probs[0, next_tokens_.cpu().item()])
            # print('--------------')
